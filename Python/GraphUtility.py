...
class GraphUtility:
    ...
    def stringifyLoopedGraph(graph):
        ...
        Runtime: O(n) Space: O(n) --> This is because we will visit each node once. And breadth first will use a stack to manage traversal that will at max hold all the elements in the graph.

        Optimized Solution:
        A much better approach comes to mind when we make use of the given constraint that there will only ever be cycles of nodes in the input graph. And all other graphs will be invalid.
        This means that, this will be a glorified doubly linked list that has a cycle and connects the last node to its first node creating a loop.
        ...
        Since we identified that a valid input will be a structure similar to a doubly linked list joined from end to its head, we will be able to weed out invalid input that is otherwise a perfectly acceptable graph, by verifying if each node only connects to two other nodes.
        Now, graphs with less than three nodes are still valid cycles but will be invalidated by our logic. So we handled those edge cases in the beginning.
        Realize that in cases of less than three nodes in the graph, the order of nodes doesn't matter.
        Runtime: O(n) Space: O(1)"""
        # If the graph is None, return an empty string
        if not graph:
            return ""
        # If the graph is two nodes or fewer, it will be our edge case
        if len(graph.keys()) <= 2:
            # In case of empty graph, this will return empty string
            # In case of one node, this will return that node in the string
            # In case of two nodes, this will return them in any order that is fine
            return "".join(graph.keys())
        # Initiate empty result
        result = []
        # Randomly determine a starting point from the adjacency list
        # Also, initiate currentNode as the node that we are processing right now
        currentNode = startNode = list(graph.keys())[0]
        # Initiate prevNode as the node that we traversed to currentNode from
        # Also initialize nextNode that we will traverse to
        nextNode = prevNode = None  # In the beginning we didn't come from anywhere
        while nextNode != startNode:
            # Process the currentNode
            result.append(currentNode)
            # Check if the current node has only two connections else throw
